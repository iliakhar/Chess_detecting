...
class ChessBoardDetecting:
    ...
    def __init__(self):
        ...
        self.conv_model.load_model(
            getcwd() + '\\lattice_points_ml\\model\\lattice_mix_130.pt')  # lattice_mix_50, lattice_rotate_detect_6_100
    ...
    def get_chessboard_img(self):
        ...
        # show_detected_board(result, self.img)
    ...
    def save_lattice_points_img(self, chessboard_img: np.ndarray):
        ...
        filename_ok = getcwd() + '\\lattice_points_ml\\latchess21\\photos\\mix\\ok_0\\'
        filename_no = getcwd() + '\\lattice_points_ml\\latchess21\\photos\\mix\\no_0\\'
        filename_border = getcwd() + '\\lattice_points_ml\\latchess21\\photos\\mix\\border_0\\'
    ...
    def show_grid(self, is_wait: bool = True):
        if self.board_grid is None:
            return
        color1: tuple[int, int, int] = (22, 173, 61)
        color2: tuple[int, int, int] = (255, 255, 0)  # const lines
        colors = [color1, color2]
        draw_lines(self.img, [self.board_grid.grid, BoardGrid.const_grid], colors, is_wait, '9')
    ...
    def show_borders(self, is_wait: bool = True):
        ...
        color2: tuple[int, int, int] = (255, 255, 0)  # const lines
        colors = [color1, color2]
        const_borders = BoardGrid.const_last_borders
        if const_borders is None:
            const_borders = []
        draw_lines(self.img, [BoardGrid.last_borders, const_borders], colors, is_wait, '8')
    ...
    def show_grupped_points(self, is_wait: bool = True, img_name: str = 'image3'):
        ...
        colors = [color1, color2, color4, color3, color6, color5]
        lt = self.lattice_points
        if lt is None:
            return
        bg = self.board_grid.bp if self.board_grid is not None else []
        points = [self.intersection_points, lt.lattice_points, lt.border_points, [BoardGrid.board_center_list[-1]], bg,
                  [BoardGrid.const_board_center]]
        draw_points(self.img, points, colors, img_name, is_wait)

def show_detected_board(result, orig_img):
    img = orig_img.copy()
    borders = result.boxes.xyxyn.cpu().numpy()
    ...
    for ind in range(len(cls)):
        ...
        cv2.line(img, (x2, y1), (x2, y2), color, 1)
        cv2.line(img, (x2, y2), (x1, y2), color, 1)
        cv2.line(img, (x1, y2), (x1, y1), color, 1)

        cv2.putText(img, cls_name, (x1, y1 - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
        cv2.putText(img, prob_str, (x2, y2 + 13),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
